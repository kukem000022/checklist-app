...
app.get("/api/profiles", async (req, res, next) => {
  try {
    const { data, error } = await req.db
      .from("profiles")
      .select("id, full_name, email, role, department, telegram_chat_id, status")
      .order("full_name");
...
app.get("/api/profiles/summary", async (req, res, next) => {
  try {
    ...
    const { data: tasks, error: taskError } = await req.db
      .from("tasks")
      .select("id, assignee_id, status, due_time");
    ...
    const summary = hydratedProfiles.map((profile) => {
      const assigned = tasks.filter((task) => task.assignee_id === profile.id);
      return {
        ...profile,
    ...
    let query = req.db
      ...
      .select(`
        *,
        ...
        task_members(user_id, profiles(id, full_name, email)),
...
app.get("/api/notifications", async (req, res, next) => {
  try {
    const { data, error } = await req.db
      ...
      .select("*, tasks(id, title, due_time, projects(id, name)), profiles(id, full_name, email)")
...
app.get("/api/daily-templates", async (req, res, next) => {
  try {
    const { data, error } = await req.db
      ...
      .select("*, projects(id, name), assignee:profiles!daily_task_templates_assignee_id_fkey(id, full_name, email)")
      .order("created_at", { ascending: false });
...
app.post("/api/tasks", async (req, res, next) => {
  try {
    ...
    if (body.list_id && body.project_id) {
      const { data: list, error: listError } = await req.db
        .from("task_lists")
      ...
      if (list.project_id !== body.project_id) {
        return res.status(400).json({ error: "Task list does not belong to selected project" });
      }
    }
    ...
    const targetChatId = project?.telegram_group_chat_id || defaultGroupChatId || assignee?.telegram_chat_id;
    ...
    if (targetChatId) {
      await sendTelegramMessage(
        ...
        [
          ...
          `Nhan su: ${assignee?.full_name || assignee?.email || "Nhan su"}`,
          `Task: ${task.title}`,
...
app.patch("/api/tasks/:taskId/detail", async (req, res, next) => {
  try {
    ...
    res.json({ ok: true, checklist_count: existingIds.length + upserts.filter((item) => !item.id).length });
  } catch (error) {
    next(error);
  }
});
