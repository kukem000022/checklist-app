...
function Sidebar({ activePage, setActivePage, profile }) {
...
function Topbar({ activePage, profile, loading, onRefresh, openModal }) {
...
function ReportsPage({ tasks, projects, profiles, notifications }) {
  const report = useMemo(() => {
    ...
    const allPeopleWithTasks = profiles
      .map((profile) => {
        const personTasks = tasks.filter((task) => task.assignee_id === profile.id);
        return {
          id: profile.id,
...
function TaskKanbanCard({ task, openTask }) {
  ...
  const initials = personName(task.assignee).slice(0, 2).toUpperCase();
      ...
      <span className="card-footer compact-card-footer">
        <span className="avatar-stack">
          ...
          {members.slice(0, 3).map((member) => (
            <span key={member.id} className="avatar-chip member-avatar">{personName(member).slice(0, 2).toUpperCase()}</span>
          ))}
...
function ProjectsPage({ projects, profiles, tasks, openModal, openTask, addMember, removeMember, updateProject }) {
  ...
  async function submitMember(event) {
    ...
    try {
      ...
      setMessage("Đã thêm thành viên dự án.");
    } catch (currentError) {
      setMessage(currentError.message);
    }
  }
  ...
  async function submitProjectTelegram(event) {
    ...
    try {
      ...
      setMessage("Da luu Telegram group chat ID cho du an.");
    } catch (currentError) {
      setMessage(currentError.message);
    }
  }
              ...
              {activeMembers.map((member) => {
                ...
                return (
                  <article key={activeProject.id + "-" + member.user_id}>
                    ...
                    <button className="tag-icon" onClick={() => removeMember(activeProject.id, member.user_id)} title="Xóa">
                      <Trash2 size={12} />
                    </button>
                  </article>
                );
...
function NotificationsPage({ dailyTemplates, projects, profiles, createDailyTemplate, updateDailyTemplate, runReminderSweepNow }) {
  ...
  async function runToday() {
    ...
    try {
      ...
      setMessage(`Đã chạy lại hôm nay. Tạo mới ${result.createdDailyTasks || 0} task, kiểm tra ${result.checked || 0} task deadline.`);
    } catch (currentError) {
      setMessage(currentError.message);
    } finally {
      setRunning(false);
    ...
    <div className="notifications-layout single-panel-layout">
      <section className="panel page-panel daily-template-panel">
        ...
        <form onSubmit={submitDailyTemplate} className="daily-template-form">
          <label>
            Dự án
            <select value={templateDraft.project_id} onChange={(event) => setTemplateDraft({ ...templateDraft, project_id: event.target.value })}>
          ...
          <button className="primary-action wide-field">
            <Plus size={17} />
            Tạo mẫu định kỳ
          </button>
        </form>
        <div className="daily-template-list">
          {dailyTemplates.map((template) => (
            <article key={template.id} className="daily-template-card">
              ...
              <button className="secondary-action compact-action" onClick={() => updateDailyTemplate(template.id, { active: !template.active })}>
...
function SettingsPage({ profile, onSaved }) {
  ...
  async function loadTelegramChatIds() {
    ...
    try {
      ...
      if (!result.ok) {
        setMessage(result.description || "Không lấy được Telegram chat ID.");
        return;
      }
      setTelegramChats(result.result || []);
      if (!result.result?.length) {
        setMessage("Chưa có chat ID. Hãy nhắn /start cho bot rồi bấm lấy lại.");
      }
    } catch (currentError) {
      setMessage(currentError.message);
    }
  ...
  async function saveAppSettings(event) {
    ...
    setMessage("Đã lưu cấu hình Telegram chung.");
  }
    ...
    <div className="split-grid">
      <section className="panel">
        ...
        <form onSubmit={saveProfile} className="stack-form">
          ...
          <button className="secondary-action" type="button" onClick={loadTelegramChatIds}><Send size={17} />Lấy chat ID từ bot</button>
          ...
          <button className="secondary-action"><Send size={17} />Lưu hồ sơ</button>
        </form>
      </section>
      ...
      <section className="panel">
        ...
        <form onSubmit={saveAppSettings} className="stack-form">
          ...
          <button className="secondary-action"><Send size={17} />Lưu Telegram chung</button>
        </form>
      </section>
      ...
      <section className="panel">
        ...
        <form onSubmit={changePassword} className="stack-form">
          ...
          <button className="secondary-action"><KeyRound size={17} />Đổi mật khẩu</button>
          {message && <p className="mini-notice">{message}</p>}
        </form>
      </section>
    </div>
          ...
          <div className="task-member-picker">
            {projectMemberProfiles.map((profile) => (
              <label key={profile.id} className="member-check-row">
                ...
                />
                <span className="avatar-chip">{personName(profile).slice(0, 2).toUpperCase()}</span>
  ...
  async function submit(event) {
    ...
    try {
      await onCreate({
        ...
        checklist: form.checklist.split("n").map((line) => line.trim()).filter(Boolean),
      });
      onClose();
    } catch (currentError) {
  ...
  return (
    <Modal title="Tạo task mới" onClose={onClose}>
      <form onSubmit={submit} className="modal-form">
        ...
        <label>Dự án<select value={form.project_id} onChange={(event) => setForm({ ...form, project_id: event.target.value })}><option value="">Cá nhân</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label>
        <label>Phụ trách<select value={form.assignee_id} onChange={(event) => setForm({ ...form, assignee_id: event.target.value })}><option value="">Tôi phụ trách</option>{profiles.map((item) => <option key={item.id} value={item.id}>{personName(item)}</option>)}</select></label>
...
function CreateUserModal({ onCreate, onClose }) {
  ...
  async function submit(event) {
    ...
    setError("");
    try {
      await onCreate({ ...form, department: form.department || null });
      onClose();
    } catch (currentError) {
      setError(currentError.message);
    }
  ...
  return (
    <Modal title="Tạo nhân sự" onClose={onClose}>
      <form onSubmit={submit} className="modal-form">
        ...
        <label>Bộ phận<input value={form.department} onChange={(event) => setForm({ ...form, department: event.target.value })} /></label>
        <label>Role<select value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value })}><option value="admin">Admin</option><option value="manager">Manager</option><option value="staff">Nhân sự</option></select></label>
        {error && <p className="form-error wide-field">{error}</p>}

        <button className="primary-action"><UsersRound size={17} />Tạo nhân sự</button>
      </form>
    </Modal>
  );
}
  ...
  return (
    <Modal title="Tạo dự án" onClose={onClose}>
      <form onSubmit={submit} className="modal-form">
        ...
        <button className="primary-action"><FolderKanban size={17} />Tạo dự án</button>
      </form>
    </Modal>
  );
}
...
function AppShell() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [profileRows, roleRows, projectRows, listRows, taskRows] = await Promise.all([
  ...
  async function createTaskList(payload) {
    await api("/api/task-lists", { method: "POST", body: JSON.stringify(payload) });
    await load();
  }
  ...
  function renderPage() {
    ...
    if (activePage === "projects") {
      return <ProjectsPage projects={projects} profiles={profiles} tasks={tasks} taskLists={taskLists} openModal={setModal} openTask={openTask} addMember={addMember} removeMember={removeMember} updateProject={updateProject} createTaskList={createTaskList} createProjectTask={createProjectTask} />;
    }
  ...
  return (
    <main className="app-layout">
      ...
      <section className="app-main">
        {activePage !== "tasks" && <Topbar activePage={activePage} profile={profile} loading={loading} onRefresh={load} openModal={setModal} />}
      ...
      {modal === "user" && <CreateUserModal onCreate={createUser} onClose={() => setModal(null)} />}
